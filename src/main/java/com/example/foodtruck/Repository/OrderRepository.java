...
public interface OrderRepository extends JpaRepository<Orders,Integer> {
    ...
    @Query("select c from Orders c where c.customer.id=?1 and c.foodTruck.id=?2")
    List<Orders> findOrdersByCustomerIdAndFoodTruckId(Integer customer_id, Integer food_id);

//    Orders findOrdersByCustomerIdAndFoodTruckId(Integer customer_id, Integer food_id);
